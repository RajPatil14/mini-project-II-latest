...
const buildShareableTrackingUrl = tripId => {
  ...
  try {
    ...
    if (!isLocalHost(apiUrl.hostname)) {
      frontendUrl.hostname = apiUrl.hostname
      frontendUrl.port = apiUrl.port || '5000'
    }
  } catch (e) {
    console.error('Unable to build network tracking URL', e)
  }
  ...
  const handleSendLink = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!isLocalHost(backendUrl.hostname)) {
          dynamicUrl = backendUrl.toString()
        }
      } catch (e) {

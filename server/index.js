...
const buildFrontendUrl = () => {
  const fallbackUrl = `http://${getLocalNetworkIp()}:${PORT}`
  ...
  try {
    ...
    if (frontendUrl.hostname === 'localhost' || frontendUrl.hostname === '127.0.0.1') {
      frontendUrl.hostname = getLocalNetworkIp()
      frontendUrl.port = PORT
    }
    return frontendUrl.origin
  } catch (error) {
    console.error('Invalid FRONTEND_URL. Falling back to local network URL.', error)
    return fallbackUrl
  }
}
...
app.post('/drivers/register', async (req, res) => {
  ...
  if (!routeId || !name || !phone) {
    return res.status(400).json({ message: 'routeId, name, and phone are required' })
  }
  ...
  if (!cleanedName || !cleanedPhone) {
    return res.status(400).json({ message: 'Valid driver name and WhatsApp number are required' })
  }
  ...
  const existingDriver = await Driver.findOne({
    $or: [
      ...
      {
        routeId,
        name: new RegExp(`^${cleanedName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i')
      }
    ]
  })
...
app.get('/locations/active/:routeId', async (req, res) => {
  ...
  try {
    ...
    const tripLocations = [];
    const activeAfter = new Date(Date.now() - ACTIVE_LOCATION_WINDOW_MS);

    for (const trip of activeTrips) {
      const latestLocation = await Location.findOne({ tripId: trip._id }).sort({ timestamp: -1 });
      if (latestLocation && latestLocation.timestamp >= activeAfter) {
        tripLocations.push({
          tripId: trip._id,
...
const { routeId, manual_count } = req.body;
  ...
  try {
    ...
    if (passengerCount > 20 && driverAvailable && busAvailable) {
